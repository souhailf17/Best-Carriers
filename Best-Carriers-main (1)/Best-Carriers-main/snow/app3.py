...
def home():
    return render_template('app.html', jours=[0,1,2,3,4,5])
...
def home1():
  ...
  if request.method == 'POST':
    ...
    # Trouver transporteurs disponibles ce jour       
    transporteurs_dispo = []
    for nom, jours in carriers[ville].items():
      if int(jour) in jours:
        transporteurs_dispo.append(nom)

    if transporteurs_dispo:
      return render_template('app.html', transporteurs=transporteurs_dispo, ville=ville, jour=jour)
      
    else:
      # Afficher jours de dispo des transporteurs
      transporteurs = []
      for nom, jours in carriers[ville].items():
        transporteurs.append({'nom':nom, 'jours': jours})
        
      return render_template('app.html', transporteurs=transporteurs, ville=ville, jour=jour)
